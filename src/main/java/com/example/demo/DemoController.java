...
public class DemoController {
    ...
    private final String defaultMessage = "Hello, Spring Boot! Version 6.1 has passed checks!";

    // GET /status returns a 200 OK response with a simple message
    @GetMapping
    public ResponseEntity<String> getStatus() {
        return ResponseEntity.ok("Status: OK, Latest version 6.1 Checksum Verified!");
    }
    ...
    public ResponseEntity<String> verifyPayload(@RequestBody Payload payload) {
        if (defaultMessage.equals(payload.getMessage())) {
            return ResponseEntity.ok("Payload verified: OK, Version 6.1 Validated!!!");
        } else {
